...
TODO:
 - loading on actions
 - figure out what's going wrong with blocking
 - confirm that the connection changing is working
*/
...
import { DirectMessageModal, ModalStyle, ModalCloseButton } from '../Modal'
...
export default function Profile() {
  ...
  async function manageConnection({ newConnectionStatus, connected_user_id, initiating_user_id }) {
    try {
      ...
      switch (newConnectionStatus) {
        ...
        case 'blocked':
          expectedStatus = 201
          // delete an existing connection, if it exists, then create a new one
          if (connection) {
      ...
      if (resp) {
        if (newConnectionStatus == 'delete') {
          setConnection(null)
        } else {
          setConnection(resp.data)
        }
        if (newConnectionStatus == 'blocked') {
          navigate('/')
        } else {
          updateSubmitWait({
            waitStatus: false,
  ...
  function FriendButtons() {
    ...
    switch (status) {
      ...
      case 'mutual':
        return (
          <>
            <ConnectionButton data-tooltip={"Remove " + profile.username + " as a buddy"} onClick={() => { manageConnection({ newConnectionStatus: 'delete', connected_user_id: profile.id, initiating_user_id: user.id }) }}><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><rect width="24" height="24" fill="none" /><path fill="currentColor" d="M15 14c2.67 0 8 1.33 8 4v2H7v-2c0-2.67 5.33-4 8-4m0-2a4 4 0 0 1-4-4a4 4 0 0 1 4-4a4 4 0 0 1 4 4a4 4 0 0 1-4 4M5 9.59l2.12-2.13l1.42 1.42L6.41 11l2.13 2.12l-1.42 1.42L5 12.41l-2.12 2.13l-1.42-1.42L3.59 11L1.46 8.88l1.42-1.42z" /></svg></ConnectionButton>
          </>
        )
