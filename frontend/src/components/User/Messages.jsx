  ...
  ul.actions {
    ...
    li {
      ...
      &:before {
        ...
        position: absolute;
        display: none;
        filter: opacity(1) drop-shadow(2px 2px 1px #ccc);
      }
    }
  }
  ul.info {
...
export function SingleMessageThread() {
  ...
  function SingleMessageView({ message }) {
    ...
    let username = (message.from_user_id == user.id ? 'you' : otherUsername)
    if (sideClass !== 'mine' && otherUsername !== '[unknown user]') {
      username = <Link to={`/profile/${otherUsername}`}>{otherUsername}</Link>;
    }
    const style = {
...
export function UserMessageBox() {
  ...
  function MessageThreadPreview({ thread, user }) {
    ...
    let usernameLink = otherUsername
    if (otherUsername !== '[unknown user]') {
      usernameLink = <Link to={`/profile/${otherUsername}`}>{otherUsername}</Link>
    }
    const from_user = (thread.most_recent_message.from_user_id == user.id ? 'you' : usernameLink)
    const to_user = (from_user == 'you' ? usernameLink : 'you')
  ...
  async function moveMessage({ thread, toLocation }) {
    try {
      const inboxStatus = (toLocation == 'inbox')
  ...
  async function loadMoreMessages() {
    ...
    const thisPageThreads = resp.data['hydra:member'].map((t) => { return t[0]; })
    const currentThreads = [...messageThreads].concat(resp.data['hydra:member'])
    setMessageThreads([...currentThreads].concat(thisPageThreads))
